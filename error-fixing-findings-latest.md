...
- **Publish-script defect found and fixed this run, but not before it fired.** The publish helper's date guard checked *format* only, not *range*, so a fail-on-purpose test with `1999-09-10` passed the guard and **executed a real PUT** — landing `error_log.csv` under the commit message `Error log fixer 1999-09-10` (commit `a5cfd68f`). The **content is correct and blob-verified** (`3ce6e74f5610`, 978,672 bytes); only the message is wrong, the same cosmetic class as the `MISSING` commit message of 2026-09-07, and it is left in place rather than churned. Two things to carry: (1) **a fail-on-purpose test must run against a side-effect-free path** — the helper now takes `--dry-run`, and the range check that `apply.py` already had was missing here, which is registry drift between two scripts that should share one guard; (2) the guard now rejects all five branches (empty, `2026-9-10`, `20260910`, `1999-09-10`, `2026-13-10`), re-tested after the fix.
